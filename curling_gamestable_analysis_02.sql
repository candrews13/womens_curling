...
order by 2 desc;
-- none have a count greater than 1

-- how many games are part of each event?
select event_id, 
...
from (  -- subquery with 'union all' combines the wins as team1 with the wins as team2
...
-- query to calculate total number of games played by each team
select team, 
	   count(*) as total_games
from ( -- subquery to list the names of team1 and team2 columns into a single column
...
from ( -- subquery counting wins as team1 plus wins as team2
	select team1 as team_name from games where team1_score > team2_score
	union all
	select team2 as team_name from games where team2_score > team1_score
	) as wins_per_team
...
select concat(team1_score, '-', team2_score) as final_score, -- concatenate the two team final scores
       count(*) as frequency
from games
